...
class DockIconMenuMultiClose(unittest.TestCase):
    ...
    class MyTestResult(runner.MyTextTestResult):
        def addError(self, test, err):
            super(DockIconMenuMultiClose.MyTestResult, self).addError(test, err)
            global result
            result = result and False

        def addFailure(self, test, err):
            super(DockIconMenuMultiClose.MyTestResult, self).addFailure(test, err)
            global result
            result = result and False

if __name__ == "__main__":
    unittest.TextTestRunner(resultclass=DockIconMenuMultiClose.MyTestResult).run(DockIconMenuMultiClose.suite())
