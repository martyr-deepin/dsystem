...
casename = 'all-5404:进程管理命令--验证对ps命令的支持'

class  Command_ps(unittest.TestCase):
    caseid = '192217'
    @classmethod
    def setUpClass(cls):
        pass
    ...
    def testPsThree(self):
        ...
        self.assertTrue(psauresult[0].startswith('USER'))
        listpsau = []
        for i in psauresult[1:]:
            listpsau.append((i.split())[-1])

        self.assertIn('-novtswitch', listpsau)
    ...
    def testPsFive(self):
        ...
        self.assertTrue(psefresult[0].startswith('UID'))
        listpsef = []
    ...
    def suite():
        ...
        suite.addTest(Command_ps('testPsOne'))
        suite.addTest(Command_ps('testPsTwo'))
        suite.addTest(Command_ps('testPsThree'))
        suite.addTest(Command_ps('testPsFour'))
        suite.addTest(Command_ps('testPsFive'))
        suite.addTest(Command_ps('testPsSix'))
        return suite

if __name__ == "__main__":
    unittest.installHandler()
    executeTestCase.runTest(Command_ps)
