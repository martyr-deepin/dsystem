...
class DockMenuRU(unittest.TestCase):
    ...
    def suite():
        ...
        suite.addTest(DockMenuRU('testOpenKeyR'))
        suite.addTest(DockMenuRU('testAppExist'))
        suite.addTest(DockMenuRU('testOpenKeyU'))
        suite.addTest(DockMenuRU('testCloseApp'))
        suite.addTest(DockMenuRU('testNotExistOnDock'))
        suite.addTest(DockMenuRU('testSetDock'))
        suite.addTest(DockMenuRU('testExistOnDock'))
        suite.addTest(DockMenuRU('testCloseLauncher'))

        # change display mode
        suite.addTest(DockMenuRU('testExChangeDisplayMode'))
    ...
    class MyTestResult(runner.MyTextTestResult):
        def addError(self, test, err):
            super(DockMenuRU.MyTestResult, self).addError(test, err)
            global result
            result = result and False

        def addFailure(self, test, err):
            super(DockMenuRU.MyTestResult, self).addFailure(test, err)
            global result
            result = result and False

if __name__ == "__main__":
    unittest.TextTestRunner(resultclass=DockMenuRU.MyTestResult).run(DockMenuRU.suite())
